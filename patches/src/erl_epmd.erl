...
%% TODO We probably don't actually want a default
%%  - we can't make any assumptions about this at all.
-define(erlang_daemon_port, 4369).
-define(epmd_dist_high, 65535).
-define(epmd_dist_low, 1024).
...
%% With mesos, we don't need to ever connect to a remote
%% EPMD (ErlPMD) because it's backed by a synced datastore
%% (currently ZK)
names(_) ->
    names().

names2(HostName) when is_atom(HostName) ->
  names1(atom_to_list(HostName));
names2(HostName) when is_list(HostName) ->
  names1(HostName);
names2(EpmdAddr) ->
  get_names(EpmdAddr).
...
do_register_node(NodeName, TcpPort) ->
	...
	{ok, Socket} ->
        ...
        %% TODO Mesos: this is where we can use Extra to sync the JSON blob
	    Extra = "",
	    Elen = length(Extra),
...
%% TODO Need a more sensible setup here. Preset defaults make little-to-no sense in mesos.
epmd_dist_high() ->
...
get_port(Node, _EpmdAddress, Timeout) ->
    error_logger:warning_msg("erl_epmd:get_port(~p, ~p, ~p)", [Node, _EpmdAddress, Timeout]),
    case open() of
	{ok, Socket} ->
	    Name = to_string(Node),
	    Len = 1+length(Name),
